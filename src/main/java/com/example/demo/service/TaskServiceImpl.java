...
public class TaskServiceImpl implements TaskService {
	...
	//普通はTaskDaoImplではなくインターフェースのTaskDaoを指定する。これにより、実装クラスが未完でもテストできる。
	//なお、インターフェースに定義されているメソッドのみしか使用できなくなる。
	...
	public Optional<Task> getTask(int id) {
		...
		//DB操作における例外は基本チェック例外になるが、Springではこれを非チェック例外としてスローしている
		try {
			return dao.findById(id);
		} catch(EmptyResultDataAccessException e) {
			throw new TaskNotFoundException("指定したタスクは存在しません");
		}
	}
